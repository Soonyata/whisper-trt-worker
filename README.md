...
Also in this repo: **`batch_pod.py`** — a pod-batch harness (prefetch pool for fetch/decode/VAD,
resume ledger, `--shard i/N` for multi-pod fan-out) that drives the same worker on rented pods
